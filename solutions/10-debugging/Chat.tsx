...
export function Chat() {
  ...
  const {
    ...
    onChunk(chunk) {
      ...
      if (chunk.type === 'CUSTOM' && chunk.name === 'workshop.middleware') {
        setTrace((items) => [...items.slice(-19), String(chunk.value.message)]);
      }
    },
  });
    ...
    <>
      ...
      <label className="faults">
        ...
        <select value={fault} onChange={(e) => setFault(e.target.value)}>
          ...
          <option value="tool-budget">Repeated tools — test budget</option>
          <option value="policy-cache">Repeated policy — test cache</option>
          <option value="tool-error">Unknown product ID</option>
        </select>
      </label>
    </>
  );
